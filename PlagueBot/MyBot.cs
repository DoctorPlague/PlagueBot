...
namespace PlagueBot
{
    class MyBot
    {
        ...
        public MyBot()
        {
            ...
            {
                var channel = e.Server.FindChannels("smonk_weed", ChannelType.Text).FirstOrDefault();
            ...
            discord.UserBanned += async (s, e) =>
            {
                ...
                await channel.SendMessage(string.Format("{0} has been banned from the server.", user.Name));
            };
        ...
        private void RegisterCommandsCommand()
        {
            commands.CreateCommand("commands").Alias(new string[] { "help", "command", "com"})
                .Do(async (e) =>
                {
                    await e.Channel.SendMessage("```Commands: !help, !rules, !meme, !prank, !nazi, !invite, !8ball, !madison, !corey, !google (eg. !google dank memes)\n\nAdmin Commands: !ban, !kick, !mute, !unmute, !delete x (x being the number of messages to delete, between 1-100)```");
                });
        }
